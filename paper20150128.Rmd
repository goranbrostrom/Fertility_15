---
title: "Repeated events and dynamic covariates"
...
Ways of analyzing fertility data (or more generally, repeated events data) are discussed. It is assumed that the fertility histories of women are collected in a dynamic way, following each woman from birth to death and noting the exact dates of her deliveries, and information of each in terms of sex of child, stillbirth, mother's civil status, socioeconomic status, 
etc. Further, links to each mother's husband (if married) and parents are normally present, which will allow us to identify siblings and and other relatives, which may be relevant in modeling family dependencies in fertility "behaviour".

But on this first occasion I will discuss the concept of *dynamic covariates* in the context of Cox regression with repeated events, by illustrating it by looking at real fertility data and by simulation. 

## Data

The data set we use for illustration is taken from the DDB, Skellefteå data. Women born 1821--1870 are followed over their fertility and presence period, that is, from age 15 until age 49 or death or out-migration, whichever comes first. Their exact age at deliveries are recorded, as well as their birthdate and civil status. Multiple births (twins) may complicate the picture I want to paint, so we will for now exclude all mothers who experience a multiple birth.

The raw data set is saved in a file named 'fert.rda'. The extension '.rda' signals that the data format is the R 'native' data format. 
...
**load** is the command to load a data file into the 'work space' of **R**. **kable** is a function in the library *knitr*, and _head_ is a function that returns a few (*six* by default) lines from the top ('head') of a data frame. **kable** 'pretty-prints' the output (a matrix).

The variables are: **id** is the DDB identification number, in this case for the mother. The data frame is organized by births for each woman and each row gives the start age (*enter*) and stop age (*exit*) for a time interval between two births. Exceptions are the first and last intervals for each woman: the first interval starts at age 15 (approximately), and the last interval ends at age 50 (with no birth), or earlier, if the woman died or in some other way was lost to follow-up (we call it *censoring*).  

For example, the woman with id `r x[1, 1]` is represented by five records (rows) in the data frame (that is what the variable **antrec** says). In the first record, the variable **parity** is zero, signifying that during this age interval (**enter** to **exit**), she has no children. At age 
`r round(x[1, 5], digits = 3)` she gives birth to a boy (seen in two ways, **event** = 1 and **sex** = man) with the DDB id (**ch.id**) `r x[1, 8]`.
...
toTime("1867-09-02")
##ibrary(skum)
skum::tillTid(18670902)
```

And, as we see at the end, the function *tillTid* in the package **skel14**. The reason for having dates as numeric is of course that it makes mathematics involving dates easy.

## The counting process approach

We define $$\{N_i(t), 15 \le t \le 50\}, \quad i \in \text{mother's id}$$ as the *number of births before and including age $t$* for woman No. $i$. By age we mean *exact age*, measured by a precision of a day, but with *time unit year*. Thus $N_i$ are *counting processes* with jumps of size one (remember, no twin births) at the age of the mother at deliveries. For mother No. $160$ it looks like this (with the code producing the figure):

```{r, echo=TRUE,fig.cap="Counting the number of births for mother with id = 160."}
...
```{r, echo=TRUE}
library(eha)
fit <- coxph(Surv(enter, exit, event > 0.5) ~ birthdate, data = fert)
kable(summary(fit)$coef, digits = 3)
```

The interpretation of the estimated regression coefficient is that the birth intensity increases by 0.18 percent per year; not a very impressive amount, but statistically *very significant*, mostly due to the large amount of births in our data.
...
```{r, echo=TRUE}
fit <- coxph(Surv(enter, exit, event > 0.5) ~ birthdate + cluster(id), data = fert)
kable(summary(fit)$coef, digits = 3)
```
...
Then, an analysis with both **period** and **civst**:

```{r cluster}
fit <- coxph(Surv(enter, exit, event > 0.5) ~ period + civst + cluster(id), data = fert)
kable(summary(fit)$coef, digits = 3)
```

What about the *proportionality assumption*? Best is to look at it graphically; first regarding **period**:

```{r propass, fig.cap = "Log cumulative hazards for two time periods, adjusted for 'civil status'."}
fit1 <- coxph(Surv(enter, exit, event > 0.5) ~ strata(period) + civst, data = fert)
plot(survfit(fit1), mark.time = FALSE, fun = "cloglog", col = c("red", "blue"), 
     xlab = "Age")
```

On this scale ("cloglog"), the curves should have a constant *absolute* distance over time, if the *PH* assumption holds. Disregarding the "random fluctuations" before age 19 (or so), we see clearly that the two curves converge, and the PH assumption is questionable. A formal test is available:

```{r zph}
cox.zph(fit1)
```

The null hypothesis is "The PH assumption is TRUE", and the $p$-value is effectively zero, so it is safe to reject the PH hypothesis. However, from the graph it looks as if the deviation is not that severe; remember that the number of births is huge, so even small effects will be detected. 

Let us look at the graph with "natural" scales:

```{r natscale, fig.cap = "Cumulative hazards for two time periods, adjusted for 'civil status'."}
plot(survfit(fit1), mark.time = FALSE, fun = "cumhaz", col = c("red", "blue"), 
     xlim = c(15, 50), xlab = "Age", ylab = "Cumulative hazards")
text(35, 2, "first period", col = "red")
text(30, 3.5, "second period", col = "blue")
abline(h = 0)
```

We would like to look at the *hazard functions* (instead of the *cumulative* ones). For non-parametric estimators, this requires somewhat complicated so-called *kernel* methods. However, with this huge amount of data, a flexible *parametric* model is satisfactory; the *piecewise constant hazard model*. We split the time interval $(15, 50])$ into 35 on-year-long sub-intervals and assume constant intensity within each sub-interval. Like this:

```{r strataper, fig.cap = "Hazard functions for two time periods, adjusted for 'civil status'."}
fit <- phreg(Surv(enter, exit, event > 0.5) ~ strata(period) + civst, data = fert, 
             dist = "pch", cuts = 16:49)
summary(fit)
...
Now let us look at **civst**:

```{r hazard, fig.cap="Hazard functions for married and unmarried, adjusted for 'period'."}
fit1 <- phreg(Surv(enter, exit, event > 0.5) ~ period + strata(civst), data = fert, 
              dist = "pch", cuts = 16:49)
summary(fit1)
plot(fit1, fn = "haz", col = c("red", "blue"), printLegend = FALSE, main = "",
     xlab = "Age", ylab = "Hazard functions")
legend("center", legend = c("married", "unmarried"), col = c("red", "blue"), lty = 1:2)
abline(h = 0)
```
...
```{r ratioplot,fig.cap="The ratio between the hazards of delivery for unmarried and married."}
...
```{r dynamic}
fit1 <- coxph(Surv(enter, exit, event > 0.5) ~ parity + period, data = fert)
kable(summary(fit1)$coef, digits = 3)
```

Note how the inclusion of **parity** deflates the importance of **period**; without **parity** we have

```{r nodynam}
fit <- coxph(Surv(enter, exit, event > 0.5) ~ period, data = fert)
kable(summary(fit)$coef, digits = 3)
```
...
fert$enter1 <- ifelse(fert$parity > 0.5, 
                      ...
                      fert$enter)
## Checking ...
kable(head(fert[, c("id", "parity", "enter", "enter1", "exit")]), digits = 3)
```
...
f <- phreg(Surv(enter, exit, event > 0.5) ~ 1, dist = "pch", cuts = 16:49, data = fert)
f1 <- phreg(Surv(enter1, exit, event > 0.5) ~ 1, dist = "pch", cuts = 16:49, data = fert)
...
Since we cannot observe the factors that give rise to this in-homogeneity, we apply a random-effects ("frailty") model. Conditionally on the random effects, it is a PH model, mut since we cannot observe the random effect, it has to be integrated out. This results in a 'population model', i.e. an average over the individuals. In **R** we can use the function **coxme** from the pachage with the same name:
...
From the output it is clear that there is a very strong random effect (women are different!). Note also tha the fixed effect for **parity** now is still very strong, but *in the opposite direction*! That is, the direction our (my?) intuition says.
...
fit <- coxph(Surv(exit - enter1, event > 0.5) ~ age + I(age^2) + as.factor(parity) + strata(id), data = fert)
```

Note the definition of **age**! Continuous covariates should *always* explicitely be *centered*, especially if one wants to include powers or interactions in the model.
...
```{r remove16, fig.cap="Effect of parity with last removed."}
...
```{r figage, fig.cap = "Effect of parity, controlled for mother's age. Effect of mother's age on fertility, controlled for parity. Reference age is 30.",echo=FALSE}
